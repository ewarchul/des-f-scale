des <- function(par,
  ...
  log.pop <- controlParam("diag.pop", TRUE)
  ...
  if (log.pop) {
    pop.log <- matrix(0, nrow = lambda, ncol = N)
  }
  if (log.bestVal) {
  ...
  if (log.pop) log$pop <- pop.log
  if (log.bestVal) log$bestVal <- bestVal.log[1:iter]
